...
「格口报修台账」页面（`/repairs`）供物业登记快递柜故障格口的报修单，并跟进完工留痕。

### 登记报修

- 在「快递柜详情」页点「登记报修」，必须选择**故障格口**（按该柜格口数量生成 1~N 号格口下拉）、填写**故障现象**和**报修人**，未填齐不能建单；纯数字格口编号不能超出该柜格口范围，后端同样拦截。
- **同一柜同一格口存在处理中报修单时不能重复建单**：登记下拉里对应格口置灰标记「处理中」，后端在建单事务内再次拦截。
- 整个登记在单个事务内一次落库，任一校验不通过整体回滚，**关掉登记窗未提交不会写出半条报修单**；前端登记窗口未提交前关闭仅丢弃草稿（已填写内容时关闭需二次确认），不产生任何数据。

### 台账列表与完工

- 报修台账列表支持按**处理中 / 已修好**状态筛选（默认处理中），并可按单号/格口/现象/报修人/柜体编号关键字搜索。
- 完工时必须填写**处理人**和**处理结果**，状态由处理中变为**已修好**并记录完工时间；已修好的单不能重复完工。完工在台账列表和柜详情「格口报修记录」卡片均可办理，完工窗未提交前关闭仅丢弃草稿。

### 报修条数与可用标记一致性

- 「快递柜列表」「快递柜详情」均显示「维修中」标记，详情页基本信息挂出处理中报修条数（维修中×N 单），并设「格口报修记录」卡片留存该柜全部报修历史；无处理中报修时详情显示「可用」。
- 「按柜报修状态」一览列出全部柜体（含停用柜），维修中的柜体置顶，逐柜展示可用标记、处理中条数与历史条数，并汇总当前处理中总条数。
- 维修中标记、可用标记、报修条数全部以 `repair_ticket` 表为唯一数据源实时推导，**刷新页面后台账列表、按柜一览、柜详情报修条数和柜体可用标记保持对得上**。

相关接口（前缀 `/api/repairs`）：

- `POST /api/repairs`：登记报修，body 为 `{ lockerId, compartmentNo, symptom, reporter, remark? }`，故障格口/现象/报修人必填
- `GET  /api/repairs?page=&size=&status=&lockerId=&keyword=`：分页报修台账，`status=PROCESSING/FIXED`
- `GET  /api/repairs/{id}`：报修单详情
- `POST /api/repairs/{id}/complete`：完工，body 为 `{ handler, repairResult }`，处理人、处理结果必填
- `GET  /api/repairs/locker/{lockerId}`：某台柜体的全部报修单（柜详情报修条数同源）
- `GET  /api/repairs/locker-overview`：按柜报修状态一览（含可用标记与处理中条数）
- `GET  /api/repairs/statuses`：报修状态字典
- 快递柜 DTO 新增 `repairing`（维修中标记）、`openRepairCount`（处理中报修条数），由报修台账实时推导

## 柜门未关告警台账模块

「柜门未关告警」页面（`/door-alarms`）供物业登记巡柜时发现的取件后柜门虚掩/未关严，并跟进确认关严留痕。

### 登记告警

- 在「快递柜详情」页点「登记柜门未关」，必须填写**上报人**；**发现未关时间**默认当前时间，补登可选过去时间（不能晚于当前时间）；**约定关严分钟数**默认 10 分钟，可按柜调整。
- **同一柜已有未处理告警时不能重复登记**：后端在登记事务内拦截，数据库函数唯一索引兜底（已关闭记录不占额度），确认关闭后才能再登记。
- 整个登记在单个事务内一次落库，任一校验不通过整体回滚，**关掉登记窗未提交不会写出半条告警**；前端登记窗口未提交前关闭仅丢弃草稿（已填写内容时关闭需二次确认），不产生任何数据。

### 告警台账与超时

- 告警台账列表支持按**未处理 / 已关闭**状态筛选（默认未处理），并可按告警编号/上报人/柜体编号关键字搜索。
- **超过约定分钟仍未关严（未处理）的柜必须出现在告警台账**：未处理记录实时计算是否已超约定分钟，超时记录带「已超时」标记，并展示已持续时长。
- 「按柜门状态」一览列出全部柜体（含停用柜），柜门未关的柜体置顶，逐柜展示未处理条数、超时标记与历史条数，并汇总当前未处理总条数与超时台数。

### 确认已关闭与一致性

- 现场核实柜门已关严后点「确认已关闭」，可填确认关闭人（留空默认“系统管理员”）和关闭说明；**同一条台账记录**状态由未处理变为已关闭并记录关闭时间，已关闭的记录不能重复关闭。
- 「快递柜列表」「快递柜详情」均显示「柜门未关」标记，详情页另设「柜门未关告警记录」卡片留存该柜全部告警历史；确认关闭后标记实时恢复。
- 未关标记、未处理条数全部以 `door_alarm_record` 表为唯一数据源实时推导，**刷新页面后柜体列表不再把该柜标成未关，告警台账按未处理或已关闭筛选找到的都是同一条记录**。

相关接口（前缀 `/api/door-alarms`）：

- `POST /api/door-alarms`：登记柜门未关，body 为 `{ lockerId, doorOpenTime?, reporter, thresholdMinutes?, remark? }`，上报人必填；同一柜已有未处理告警返回 400
- `GET  /api/door-alarms?page=&size=&status=&lockerId=&keyword=`：分页告警台账，`status=OPEN/CLOSED`
- `GET  /api/door-alarms/{id}`：告警详情
- `POST /api/door-alarms/{id}/close`：确认已关闭，body 为 `{ closeOperator?, closeNote? }`，已关闭不能重复操作
- `GET  /api/door-alarms/locker/{lockerId}`：某台柜体的全部告警记录（柜详情告警记录同源）
- `GET  /api/door-alarms/locker-overview`：按柜门状态一览（含未关标记与未处理条数）
